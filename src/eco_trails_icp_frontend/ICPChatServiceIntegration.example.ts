...
async function handleChatSubmit(event: Event): Promise<void> {
    ...
    if (message.length > MAX_MESSAGE_LENGTH) {
        showErrorNotification(`Съобщението е твърде дълго (макс ${MAX_MESSAGE_LENGTH} символа)`);
        return;
    }
    ...
    try {
        ...
        // Show error message
        displayAIResponse({
            response: 'Извинявам се, възникна техническа грешка. Моля, опитайте отново след малко.',
            coords: null
        });
    }
}
 ...
 * }
 */
async function getTrailByIdICP(trailId: number | bigint): Promise<TrailRecord | null> {
    if (!chatService || !chatService.isInitialized()) {
        console.error('Chat service not initialized');
        return null;
    }
 ...
 */
function createMessageElement(content: string, type: 'user' | 'ai'): HTMLDivElement {
...
Как мога да ви помогна днес?`,
        ...
        showErrorNotification('Неуспешно зареждане на приложението. Моля, презаредете страницата.');
    }
});
