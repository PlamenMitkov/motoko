...
export interface BackendActor {
    queryData: (userId: string, message: string) => Promise<{
        response: string;
        coords: [] | [Coordinates];
    }>;
...
export class ICPChatService {
    ...
    constructor(config: ICPChatServiceConfig = {}) {
        this.config = {
            ...
            local: config.local !== undefined ? config.local : this._isLocalEnvironment(),
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            // Create the actor
            this.actor = Actor.createActor<BackendActor>(idlFactory as IDL.InterfaceFactory, {
                agent: this.agent,
            ...
            throw new Error(`ICPChatService initialization failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    private _ensureInitialized(): void {
        if (!this.initialized || !this.actor) {
            throw new Error('ICPChatService not initialized. Call initialize() first.');
        }
    }
    ...
    public async sendMessage(message: string): Promise<ChatResponse> {
        ...
        try {
            ...
            throw new Error(`Failed to communicate with canister: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    public async searchTrails(query: string): Promise<TrailRecord[]> {
        ...
        try {
            ...
            throw new Error(`Trail search failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
     ...
     * }
     */
    public async getTrailById(trailId: number | bigint): Promise<TrailRecord | null> {
        this._ensureInitialized();
        ...
        try {
            ...
            throw new Error(`Get trail failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    public async listAllTrails(): Promise<TrailRecord[]> {
        ...
        try {
            ...
            throw new Error(`List trails failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
